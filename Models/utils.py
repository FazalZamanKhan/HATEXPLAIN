...
def _env_data_file(default_path):
    return os.environ.get('HATEXPLAIN_DATA_FILE', default_path)


def _env_class_file(default_path):
    return os.environ.get('HATEXPLAIN_CLASS_FILE', default_path)
...
def return_params(path_name,att_lambda,num_classes=3):
    ...
    params['data_file'] = _env_data_file(dict_data_folder[str(params['num_classes'])]['data_file'])
    params['class_names'] = _env_class_file(dict_data_folder[str(params['num_classes'])]['class_label'])
