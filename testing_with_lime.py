...
if __name__=='__main__': 
    ...
    params['data_file']=os.environ.get('HATEXPLAIN_DATA_FILE', dict_data_folder[str(params['num_classes'])]['data_file'])
    params['class_names']=os.environ.get('HATEXPLAIN_CLASS_FILE', dict_data_folder[str(params['num_classes'])]['class_label'])
    temp_read=get_annotated_data(params)
    
    with open(os.environ.get('HATEXPLAIN_SPLIT_FILE', 'Data/post_id_divisions.json'), 'r') as fp:
    ...
    params['variance']=1
    params['device']='cpu'
    params['data_file']=os.environ.get('HATEXPLAIN_DATA_FILE', params['data_file'])
    params['class_names']=os.environ.get('HATEXPLAIN_CLASS_FILE', params['class_names'])
    fix_the_random(seed_val = params['random_seed'])
    test_data=get_test_data(temp_read,params,message='text')
    final_dict=get_final_dict_with_lime(params,model_to_use,test_data,topk=5)
    path_name=model_dict_params[model_to_use]
    output_suffix = os.environ.get('HATEXPLAIN_OUTPUT_SUFFIX', '').strip()
    if output_suffix:
        output_suffix = ''.join(ch if (ch.isalnum() or ch in ['_', '-']) else '_' for ch in output_suffix)
        path_name_explanation='explanations_dicts/'+path_name.split('/')[1].split('.')[0]+'_explanation_with_lime_'+str(params['num_samples'])+'_'+str(params['att_lambda'])+'_'+output_suffix+'.json'
    else:
