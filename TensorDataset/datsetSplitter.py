...
def _get_split_file():
    return os.environ.get('HATEXPLAIN_SPLIT_FILE', 'Data/post_id_divisions.json')


def _get_cache_filename(params):
    filename = set_name(params)
    cache_suffix = os.environ.get('HATEXPLAIN_CACHE_SUFFIX', '').strip()
    if cache_suffix:
        safe_suffix = ''.join(ch if (ch.isalnum() or ch in ['_', '-']) else '_' for ch in cache_suffix)
        filename = filename[:-7] + '_' + safe_suffix + '.pickle'
    return filename
...
def createDatasetSplit(params):
    filename = _get_cache_filename(params)
    dataset = None
    if path.exists(filename):
        ...
        default_filename = set_name(params)
        backup_filename = None
        restore_default_cache = False
        remove_generated_default = False

        if filename != default_filename:
            backup_filename = default_filename + '.hatexplain_cache_backup'
            if path.exists(default_filename):
                shutil.copy(default_filename, backup_filename)
                restore_default_cache = True
            else:
                remove_generated_default = True

        try:
            dataset = collect_data(params)
            if filename != default_filename and path.exists(default_filename):
                shutil.copy(default_filename, filename)
        finally:
            if filename != default_filename:
                if restore_default_cache and backup_filename and path.exists(backup_filename):
                    shutil.move(backup_filename, default_filename)
                elif remove_generated_default and path.exists(default_filename):
                    os.remove(default_filename)

                if backup_filename and path.exists(backup_filename):
